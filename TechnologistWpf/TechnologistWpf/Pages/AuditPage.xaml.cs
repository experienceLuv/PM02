...
namespace TechnologistWpf.Pages
{
    public partial class AuditPage : UserControl
    {
        ...
        private async Task LoadData()
        {
            try
            {
                var url = $"/api/audit-logs?page={_page}&pageSize={PageSize}";
                var response = await ApiClient.Instance.GetAsync<PaginatedResponse<AuditLog>>(url);
                DgAudit.ItemsSource = response.Items;
